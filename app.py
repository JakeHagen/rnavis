...
def data():
    ...
    try:
        rna_exp = ge.gene_expression(pandas.read_sql_table(table + "_voom",
                                        index_col = 'gene', con = engine))
        results = ge.pca_json(rna_exp, voom = False)
    except:
        rna_exp = ge.gene_expression(pandas.read_sql_table(table,
                                        index_col = 'gene', con = engine))
...
def batch():
    ...
    table = j['table']
    rna_exp = ge.gene_expression(pandas.read_sql_table(table + "_voom", index_col = 'gene',
                                    con = engine))
